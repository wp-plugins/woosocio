...
if ($fb_user) {
	...
	$user_groups = $woosocio -> facebook -> api("/me/groups");
  	...
  	$loginUrl = $woosocio->facebook->getLoginUrl(array('scope' => 'manage_pages, publish_actions, user_managed_groups'));
}
  ...
  <div id="woosocio-services-block">
	<div class="woosocio-service-entry" >
		...
		?>
            ...
            <input type="radio" name="pages" value="<?php echo $group['id'] ?>" <?php echo ($fb_page_value == $group['id']) ? 'checked':''?>><?php echo $group['name'] ?><br>
        <?php
        }
		}	//$woosocio->pa($user_profile);	
		?>
    ...
    <div class="woosocio-service-entry" style="font-size:18px;">
    <?php
        if($fb_user!==0){
			/*
				Check permissions
