...
class Woo_Socio {
	...
	public function socialize_post($post_id){
		...
		if ( $this->check_connection() && $fb_post && !$socio_post->ID == '' )
		{
			...
			try {
				//$page_info = $this -> facebook -> api("/$fb_page_value?fields=access_token");
	        	/*$ret_obj = $this->facebook->api('/me/feed', 'POST', array( 'access_token'  => $page_info['access_token'],
																			 'link' => $socio_link,
                                         									 'message' => $message)
                                      									  );*/
	...
	public function products_list () {
		...
		//add_action('load-edit.php', array($this, 'woosocio_product_list') );
		
		$url = admin_url('edit.php');
    	wp_safe_redirect( add_query_arg( array('post_type' => 'product',
											   'list'	   => 'woosocio'), $url) );
		/*add_filter( 'manage_product_posts_columns' , array($this, 'book_cpt_columns'));
		add_action( 'manage_product_posts_custom_column', array($this, 'woosocio_custom_product_columns'), 2 );*/
		/*$filepath = $this->plugin_path.'woosocio.products_list.php';
		if (file_exists($filepath))
			include_once($filepath);
		else
			die('Could not load file '.$filepath);*/
	}
	public function woosocio_product_list () {
